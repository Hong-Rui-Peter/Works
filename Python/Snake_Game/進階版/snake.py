...
class Snake:
    ...
    def move(self):
        ...
        while self.move_angle_list:
            ...
            if (
                next_move_angle != self.head.heading()
                and next_move_angle != (self.head.heading() + 180) % 360
            ):
    ...
    def is_collision_with_wall(self, width, height):
        if (
            self.head.xcor() > (width / 2 - 30)
            or self.head.xcor() < -(width / 2 - 30)
            or self.head.ycor() > (height / 2 - 50)
            or self.head.ycor() < -(height / 2 - 30)
        ):
            return True
        return False
